...
# # # CREATE DEPENDANCIES FOR FACE RECOGNITION
...
def train_lbph(training_features, training_labels):
    # Train the classifier using the training set
    lbphRecognizer.train(training_features, np.asarray(training_labels));  
    # Return the trained classifier
    return lbphRecognizer;

def train_fishface(training_features, training_labels):
    # Train the classifier using the training set
    fishface.train(training_features, np.asarray(training_labels));  
    # Return the trained classifier
    return fishface;
...
def train_test_split(rand_seed, print_matrix):
    ...
    training_features, testing_features, training_labels, testing_lables = ms.train_test_split(list_of_images, list_of_labels, test_size=0.2, random_state = rand_seed);
    # Create the svm
    classifier = train_fishface(training_features, training_labels);
...
workbook = xlsxwriter.Workbook('fishfaceprecognizer.xlsx');
...
classifier = train_lbph(list_of_images, list_of_labels);
